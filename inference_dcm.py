...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    draw.multiline_text((10, 90),
                        "Patient ID: {}\nAnterior volume: {}\nPosterior volume: {}\nTotal volume:  {}".format(header.PatientID, inference['anterior'], inference['posterior'], inference['total']),
                        (255, 255, 255), font=main_font)
    draw.multiline_text((10, 250),
                        'Original:',
                        (255, 255, 255), font=main_font)
    draw.multiline_text((560, 250),
                        'Predicted:',
                        (255, 255, 255), font=main_font)
    ...
    # unpad prediction
    slice_tuple = tuple(slice(dim) for dim in orig_vol.shape)
    pred_vol = pred_vol[slice_tuple]
    
    slice_ = orig_vol[orig_vol.shape[0]//2]
    pred_slice = pred_vol[pred_vol.shape[0]//2]
    # Create a PIL image from array:
    # Numpy array needs to flipped, transposed and normalized to a matrix of values in the range of [0..255]
  
    nd_img = np.flip((slice_/np.max(slice_))*0xff).T.astype(np.uint8)
...
def get_series_for_inference(path):
    ...
    for directory in os.listdir(path):
        ...
        dicoms.extend([pydicom.dcmread(os.path.join(subdir, f)) for f in os.listdir(subdir)]) 
    ...
    # tain way. Can you figure out which is that? 
...
if __name__ == "__main__":
    ...
    os_command("storescu localhost 4242 -v -aec HIPPOAI {}".format(report_save_path))
